...
- Ask for city, dates/nights, and number of guests before recommending. But be conversational — if they give you multiple pieces of info at once, don't re-ask.
- Sort recommendations by guest rating (highest first).
- The logged-in user is ${TRAVELER.name}, a ${TRAVELER.loyalty.tier} member of ${TRAVELER.loyalty.program}. Greet them by first name and acknowledge their loyalty status when relevant. You do NOT have access to their preferences, budget, or travel history.
...
function parseResponse(text) {
  const result = { text: text, hotelIds: null, detailId: null, booking: null, confirmed: false };
...
function DateRange({ nights }) {
  if (!nights) return null;
  const { checkIn, checkOut } = getCheckInOut(nights);
  return <span>{fmtDate(checkIn)} → {fmtDate(checkOut)}</span>;
}
...
function HotelCard({ hotel, nights, onSelect, allHotels }) {
  ...
  return (
    <div onClick={() => onSelect(hotel)} style={{ background: "white", borderRadius: 12, overflow: "hidden", border: "1px solid #e5e7eb", cursor: "pointer", transition: "box-shadow 0.2s, transform 0.2s", minWidth: 260, maxWidth: 340, flex: "1 1 auto", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}
      ...
      <div style={{ padding: "12px 14px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
          ...
          <div onClick={() => setCancelType("refundable")}
            ...
            <span style={{ fontSize: 12, color: "#6b7280" }}>+${refundableExtra}</span>
          </div>
        </div>
...
function HotelDetailView({ hotel, nights, onSelectRoom, onBack }) {
  const n = nights || 3;
  const taxRate = CITY_TAX[hotel.city] || 0.12;
  ...
  return (
    <div style={{ background: "white", borderRadius: 12, overflow: "hidden", border: "1px solid #e5e7eb", marginTop: 8 }}>
      ...
      <div style={{ padding: "16px 18px" }}>
        ...
        <div style={{ fontWeight: 700, fontSize: 16, color: "#1f2937", marginBottom: 12 }}>Choose your room</div>
...
function BookingSummary({ hotel, room, nights, guests, total, onConfirm }) {
  ...
  const Section = ({ children, style: s }) => (
    <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: "20px", marginBottom: 12, background: "white", ...s }}>{children}</div>
  );

  const Collapsible = ({ title, open, toggle, children }) => (
          ...
          <Section>
            ...
            <div style={{ display: "flex", gap: 14, marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "#0f766e", fontWeight: 500, marginTop: 3 }}>📅 <DateRange nights={nights} /></div>
                <Stars count={hotel.star} />
              </div>
            </div>
      ...
      {step === "checkout" && (
        <div>
          ...
          <div style={{ padding: "10px 14px", background: "#f0fdfa", borderRadius: 10, marginBottom: 14, display: "flex", gap: 12, alignItems: "center", fontSize: 13, color: "#0f766e", fontWeight: 500, flexWrap: "wrap" }}>
            ...
            <span>·</span>
            <span>📅 <DateRange nights={nights} /></span>
            <span>·</span>
...
function TripContext({ city, nights, guests }) {
  if (!city) return null;
  return (
    <div style={{ padding: "8px 20px", background: "#f0fdfa", borderBottom: "1px solid #ccfbf1", display: "flex", gap: 16, alignItems: "center", fontSize: 13, color: "#0f766e", fontWeight: 500, flexShrink: 0, flexWrap: "wrap" }}>
      {city && <span>📍 {city}</span>}
      {nights && <span>📅 <DateRange nights={nights} /></span>}
...
function ConfirmationView({ hotel, room, nights, guests, total, taxes, fees, discount, grandTotal, confirmationId, milesEarned, milesUsed }) {
  ...
  return (
    <div style={{ background: "white", border: "1px solid #ccfbf1", borderRadius: 12, overflow: "hidden", marginTop: 8 }}>
      ...
      <div style={{ padding: 18, display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}><span style={{ color: "#52525b" }}>Dates</span><span style={{ fontWeight: 600, color: "#18181b" }}><DateRange nights={nights} /></span></div>
...
export default function HTSBookingAssistant() {
  ...
  useEffect(() => {
    ...
    setMessages([{
      ...
      quick: ["I'm heading to Chicago", "Austin, 3 nights, 2 guests", "Surprise me"],
    }]);
  }, []);
  ...
  async function send(text) {
    ...
    const rawResponse = await callLLM(newHistory);
    ...
    if (parsed.booking) {
      if (parsed.booking.nights) setLastNights(parsed.booking.nights);
      if (parsed.booking.guests) setLastGuests(parsed.booking.guests);
    }

    // Try to extract nights/city/guests from conversation
    const nightsMatch = userText.match(/(\d+)\s*night/i);
    if (nightsMatch) setLastNights(parseInt(nightsMatch[1]));
    ...
    if (parsed.booking) {
      ...
      if (hotel && room) {
        const nights = parsed.booking.nights || lastNights;
        const guests = parsed.booking.guests || lastGuests;
        botMsg.bookingData = { hotel, room, nights, guests, total: room.price * nights };
  ...
  function handleRoomSelect(hotel, room) {
    ...
    const bookingData = { hotel, room, nights, guests, total: (room.effectivePrice || room.price) * nights };
  ...
  return (
    <div style={{ width: "100%", maxWidth: 860, margin: "0 auto", height: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Inter', system-ui, sans-serif", background: "white" }}>
      ...
      <TripContext city={lastCity} nights={lastNights} guests={lastGuests} />

      <div ref={chatRef} style={{ flex: 1, overflowY: "auto", padding: "20px 16px", display: "flex", flexDirection: "column", gap: 16 }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.from === "user" ? "flex-end" : "flex-start", animation: "fadeUp 0.3s ease" }}>
            {msg.from === "bot" ? (
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start", maxWidth: "92%" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
                  ...
                  {msg.hotels && (
                    <div style={{ width: "100%" }}>
                      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>{msg.hotels.length} properties available · Total price includes taxes and fees</div>
                      <ScrollCarousel>
                        {msg.hotels.map(h => (
                  ...
                  {msg.hotelDetail && <HotelDetailView hotel={msg.hotelDetail} nights={lastNights} onSelectRoom={handleRoomSelect} onBack={handleBack} />}
